#==========================================================
# A01370082 IRVIN EMMMANUEL TRUJILLO DÍAZ
#==========================================================
...
def prime_products(n)
  product = 1
 (2..n-1).each { |i|  product *= i if i.prime?}
  product
end
...
class RandomNumberGenerator
  attr_accessor :name, :seed, :min, :max, :originalSeed
  include Observable
  ...
  def each
    @seed = @originalSeed
    restartSeed
    randomValue = @seed % (@max-@min+1)+@min
    while true
      randomValue = @seed % (@max-@min+1)+@min
      for observer in @observers
        update(observer ,randomValue)
      end
      yield randomValue
  ...
  def update(followingName,number)
      puts ("#{followingName.getName} received #{number} from #{@name}\n")
  end
